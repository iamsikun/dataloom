...
# --- Confirm import path ---
spec = importlib.util.find_spec("dataloom")
if spec and getattr(spec, "submodule_search_locations", None):
    pkg_dir = list(spec.submodule_search_locations)[0]
    print(f"✅ Using dataloom from: {pkg_dir}")
else:
    print(
        "❌ Could not import dataloom. Ensure src/dataloom exists or install with 'pip install -e .'"
    )
